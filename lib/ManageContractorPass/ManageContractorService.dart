...
class ManageContractorService {
  ...
  Future<void> ensureSheetsConnected() async {
    if (GoogleSheets.contractorPassPage == null) {
  ...
  Future<bool> addContractor(String name) async {
    ...
    try {
      ...
      if (name.isNotEmpty) {
        await GoogleSheets.contractorPassPage!.values.appendRow([name]);
        return true;
      } else {
    ...
    try {
      ...
      for (int i = 0; i < rows.length; i++) {
        if (rows[i][0] == previousName) {
          ...
          await GoogleSheets.contractorPassPage!.values.insertValue(
            newName,
    ...
    try {
      ...
      for (int i = 0; i < rows.length; i++) {
        if (rows[i][0] == name) {
          ...
          await GoogleSheets.contractorPassPage!.deleteRow(i + 1);
          return true;
        }
      }
