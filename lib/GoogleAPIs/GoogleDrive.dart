import 'dart:io' as io;

import 'package:flutter/foundation.dart' show kIsWeb;
import 'package:googleapis/drive/v3.dart' as drive;
import 'package:googleapis_auth/auth_io.dart';
import 'package:image_picker/image_picker.dart';

import '../GoogleAPIs/GoogleDriveWeb.dart';

class GoogleDrive {
  GoogleDriveWeb googleDriveWeb = GoogleDriveWeb();

  final _credentials = ServiceAccountCredentials.fromJson({
    "type": "service_account",
    "project_id": "pb-security-system-b3f0a",
    "private_key_id": "b2ba460d4c123e68ef47dc82707b6a39ebfc10f6",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n7H3Wgv/mlhMCwfKvVgKNELim\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-gwryx@pb-security-system-b3f0a.iam.gserviceaccount.com",
    "client_id": "107350847642804765683",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-gwryx%40pb-security-system-b3f0a.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
  });

  final _scopes = [drive.DriveApi.driveScope];

  Future<List<String>> getGoogleDriveFilesUrl(
      List<XFile> files, String? folderID) async {
    if (kIsWeb) {
      return googleDriveWeb.getGoogleDriveFilesUrlWeb(files, folderID!);
    } else {
      return getGoogleDriveFilesUrlNonWeb(files, folderID!);
    }
  }

  Future<List<String>> getGoogleDriveFilesUrlNonWeb(
      List<XFile> files, String folderID) async {
    try {
      // Ensure folderID is not null
      if (folderID.isEmpty) {
        throw Exception('Invalid folder ID');
      }

      final client = await clientViaServiceAccount(_credentials, _scopes);
      var driveApi = drive.DriveApi(client);

      // Create a list to hold the futures
      List<Future<String>> futures = [];

      for (var pickedFile in files) {
        // Create a future for each file upload
        futures.add(() async {
          final file = drive.File();
          file.name = pickedFile.name;
          file.parents = [folderID];

          final localFile = io.File(pickedFile.path);
          final fileBytes = await localFile.readAsBytes();

          final result = await driveApi.files.create(
            file,
            uploadMedia:
                drive.Media(Stream.fromIterable([fileBytes]), fileBytes.length),
          );

          if (result.id == null) {
            throw Exception('File upload failed, no ID returned');
          }

          // Return the file's ID (or you can return webContentLink if needed)
          return result.id!;
        }());
      }

      // Wait for all futures to complete and return the results
      return await Future.wait(futures);
    } catch (e) {
      print('An error occurred: $e');
      rethrow;
    }
  }

  Future<String> uploadFileToFolder(
      io.File file, String fileName, String mimeType, String folderId) async {
    try {
      // Ensure folderId is not null
      if (folderId.isEmpty) {
        throw Exception('Invalid folder ID');
      }

      final client = await clientViaServiceAccount(_credentials, _scopes);
      var driveApi = drive.DriveApi(client);

      final driveFile = drive.File();
      driveFile.name = fileName;
      driveFile.parents = [folderId];

      final fileBytes = await file.readAsBytes();

      final result = await driveApi.files.create(
        driveFile,
        uploadMedia:
            drive.Media(Stream.fromIterable([fileBytes]), fileBytes.length),
      );

      if (result.id == null) {
        throw Exception('File upload failed, no ID returned');
      }

      // Return the file's webContentLink or ID, depending on your needs
      return result.id!;
    } catch (e) {
      print('An error occurred while uploading file: $e');
      rethrow;
    }
  }
}
