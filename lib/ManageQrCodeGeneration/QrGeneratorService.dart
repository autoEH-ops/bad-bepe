...
class QrGeneratorService {
  ...
  Future<void> ensureSheetsConnected() async {
    if (GoogleSheets.qrReportPage == null) {
      await GoogleSheets
          .connectToReportList(); // Ensure connection to Google Sheets
    }
  }
  ...
  Future<void> sendEmail(String receiverEmail, String messageContent) async {
    ...
    await gmailAPI.sendEmail(
        receiverEmail, messageContent); // Send email using GmailAPI
  }
...
// Validate email
    ...
    if (savedData == null) {
      print(
          "Failed to verify data in Google Sheets. No message will be sent.");
      return;
    }
  ...
  Future<Map<String, dynamic>?> getVerifiedDataFromGoogleSheets(
    ...
    print("Total rows fetched: ${rows
        .length}"); // Display only the total number of rows
    ...
    for (var row in rows) {
      ...
      String sheetPhoneNumber = row[4]
          .toString()
          .trim(); // Column E for phone number
  ...
  bool isValidEmail(String email) {
    print(
        "Validating email: '$email'"); // Debug statement for email validation
  ...
  Future<void> logToSheet(String message) async {
    ...
    if (logSheet == null) {
      print('LOGPAGE is not initialized!');
      return;
    }
    ...
    try {
      ...
      if (message != "Data added successfully to Google Sheets!") {
        ...
        print('Log entry saved to Google Sheets: $message');
      } else {
        print('Skipping log entry for: $message');
      }
    } catch (e) {
      print('Error saving log to Google Sheets: $e');
    }
  }
