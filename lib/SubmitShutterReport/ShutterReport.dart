...
import 'package:created_by_618_abdo/SubmitShutterReport/SuperAdminSettings.dart';
import 'package:flutter/foundation.dart' show kIsWeb;
import 'package:flutter/material.dart';
import 'package:image_picker/image_picker.dart';

import '../SubmitShutterReport/ShutterReportService.dart';

class SubmitShutterReport extends StatefulWidget {
  const SubmitShutterReport(
      {super.key, required this.email, required this.role});
  final String email;
  final String role;
  @override
  State<SubmitShutterReport> createState() => _SubmitShutterReportState();
}

class _SubmitShutterReportState extends State<SubmitShutterReport> {
  ShutterReportService shutterReportService = ShutterReportService();

  ShutterReportSuperAdminSettings shutterReportSuperAdminSettings =
      ShutterReportSuperAdminSettings();
  ...
  void _loadData() async {
    await shutterReportService.checkAndCreateDocument();
    await shutterReportSuperAdminSettings.checkAndCreateDocument();

    nextShutterPoint = await shutterReportService.getCurrentShutter();
    shutterStatus = await shutterReportService.getCurrentShutterStatus();
    latestData = await shutterReportService.getLastRow();

    title = await shutterReportSuperAdminSettings.getTitleAndRequired();
    ...
    setState(() {
      ...
      required6 = title["Required 6"] ??
          false; // Use the null-aware operator to provide a default value
      required7 = title["Required 7"] ?? false;
  ...
  String nextShutterPoint = "";

  String shutterStatus = "";

  Map<String, dynamic> latestData = {};

  List<XFile> shutterImages = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Shutter Report Page"),
                      ...
                      Padding(
                        ...
                        child: TextFormField(
                          ...
                          onChanged: (value) async {
                            await shutterReportSuperAdminSettings
                      ...
                      Column(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Text(
                              "${title["Title 2"] ?? ''} $nextShutterPoint", // Use the null-aware operator to provide a default value
                              style: const TextStyle(
                                fontSize: 14.0,
                              ),
                            ),
                          ...
                          Padding(
                            padding: const EdgeInsets.only(left: 10),
                            child: Text(
                              "${title["Title 3"] ?? ''} $shutterStatus", // Use the null-aware operator to provide a default value
                              style: const TextStyle(
                                fontSize: 14.0,
                              ),
                            ),
                          ...
                          Padding(
                            padding: const EdgeInsets.only(left: 10),
                            child: Text(
                              "${title["Title 4"] ?? ''} ${latestData.isNotEmpty ? '${latestData['Previous Shutter']}' : 'No data yet'}", // Use the null-aware operator to provide a default value
                            ),
                          ),
                          Padding(
                            padding: const EdgeInsets.only(left: 10),
                            child: Text(
                              "${title["Title 5"] ?? ''} ${latestData.isNotEmpty ? '${latestData['Prev Shutter Date']} ${latestData['Prev Shutter Time']}' : 'No data yet'}", // Use the null-aware operator to provide a default value
                            ),
                          ),
                        ],
                      ),
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: GridView.builder(
                          ...
                          itemCount: shutterImages.length,
                            ...
                            return Stack(
                              ...
                              children: [
                                GestureDetector(
                                    ...
                                    showDialog(
                                      ...
                                      builder: (BuildContext context) {
                                        return AlertDialog(
                                          content: kIsWeb
                                              ? Image.network(
                                                  shutterImages[index].path,
                                                  fit: BoxFit.contain,
                                                )
                                              : Image.file(
                                                  File(shutterImages[index]
                                                      .path),
                                                  fit: BoxFit.contain,
                                                ),
                                        );
                                  ...
                                  child: kIsWeb
                                      ? Image.network(shutterImages[index].path)
                                      : Image.file(
                                          File(shutterImages[index].path),
                                          fit: BoxFit.cover,
                                        ),
                                ),
                                IconButton(
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  try {
                    bool success = await shutterReportService.addShutterReport(
                        widget.email,
                        nextShutterPoint,
                        shutterStatus,
                        shutterImages,
  ...
  Future<List<XFile>> _pickImage() async {
    ...
    setState(() {
      ...
      shutterImages.addAll(pickedImages);
    });
    return pickedImages;
  }
  ...
  navigateToSubmitShutterReport() {
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => SubmitShutterReport(
                  email: widget.email,
                  role: "Security",
                )));
  }
