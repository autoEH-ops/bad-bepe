...
import 'ManageDriveService.dart';

class ManageDrivePage extends StatefulWidget {
  final GoogleSheets googleSheetsApi = GoogleSheets();

  ManageDrivePage({super.key});

  @override
  State<ManageDrivePage> createState() => _ManageContractorPageState();
}

class _ManageContractorPageState extends State<ManageDrivePage> {
  final TextEditingController textController = TextEditingController();
  List<String> contractorNames = [];
  ManageDriveService manageContractorService = ManageDriveService();
      ...
      body: FutureBuilder<List<String>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(
              child: Text('Error loading Drive',
                  style: TextStyle(color: Colors.white70)),
            );
          } else if (snapshot.data == null || snapshot.data!.isEmpty) {
            return const Center(
              child: Text('No Drive found',
                  style: TextStyle(color: Colors.white70)),
            );
          } else {
  ...
  void addContractorPopUp() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1f293a),
        title:
            const Text('Add Drive', style: TextStyle(color: Colors.white)),
        content: TextField(
          controller: textController,
          style: const TextStyle(color: Colors.white),
          decoration: InputDecoration(
            hintText: 'Enter Drive',
