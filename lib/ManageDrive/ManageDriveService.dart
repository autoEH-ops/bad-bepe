...
class ManageDriveService {
  ...
  Future<void> ensureSheetsConnected() async {
    if (GoogleSheets.drivePasslist == null) {
  ...
  Future<bool> addContractor(String name) async {
    ...
    try {
      ...
      if (name.isNotEmpty) {
        await GoogleSheets.drivePasslist!.values.appendRow([name]);
        return true;
      } else {
        return false; // Name is empty
      }
    } catch (e) {
      print("Error adding Drive: $e");
    ...
    try {
      ...
      for (int i = 0; i < rows.length; i++) {
        if (rows[i][0] == previousName) {
          ...
          await GoogleSheets.drivePasslist!.values.insertValue(
            newName,
    ...
    try {
      ...
      for (int i = 0; i < rows.length; i++) {
        if (rows[i][0] == name) {
          ...
          await GoogleSheets.drivePasslist!.deleteRow(i + 1);
          return true;
        }
      }
